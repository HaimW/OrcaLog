...
export default function RegisterPage() {
  ...
  const inputClass = "w-full py-3 rounded-xl text-sm font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-teal-400/50";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      <div
        ...
        style={{ background: "linear-gradient(160deg, #030f18 0%, #071e2e 40%, #0B4F6C 75%, #1B98A6 100%)" }}
      />
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.6)" }}>{t("auth.registerTitle")}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Full name */}
          <div className="relative">
            <User size={15} className="absolute start-3.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: "rgba(255,255,255,0.4)" }} />
            <input
              type="text"
        ...
        <p className="mt-6 text-center text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
          {t("auth.hasAccount")}{" "}
          <Link href="/login" className="font-semibold underline underline-offset-2" style={{ color: "rgba(255,255,255,0.85)" }}>
            {t("auth.loginBtn")}
          </Link>
        </p>
      </div>
