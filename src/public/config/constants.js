/**
 * OrcaLog — Predefined Bilingual Constants
 *
 * All lists follow the registry pattern: { id, he, en }.
 * To add a new item, simply append to the array — no other changes needed.
 */
...
// ─── Helper: Get localized label from a bilingual list ────────

/**
 * Returns the localized label for an item in a bilingual list.
 * @param {Array<BilingualItem>} list
 * @param {string} id
 * @param {string} lang - 'he' or 'en'
 * @returns {string}
 */
export function getLabel(list, id, lang) {
...
/**
 * Returns all items in a bilingual list with labels for the given language.
 * @param {Array<BilingualItem>} list
 * @param {string} lang - 'he' or 'en'
 * @returns {Array<{id: string, label: string}>}
 */
export function getLocalizedList(list, lang) {
  return list.map(item => ({
    id: item.id,
    label: item[lang] || item.en || item.id,
  }));
}
